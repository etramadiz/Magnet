// lengkapi-profil.js (versi Firestore)
...
import { MagnetDB } from './db.js'; // untuk session & fallback
...
// Helper toast (ambil dari global)
const showToast = window.showToast || function(msg) { alert(msg); };
...
async function updateProgress() {
  // Data dari Firestore sudah ada di variabel global? Kita ambil ulang dari state form
  const nama = document.getElementById('f-nama')?.value.trim() || '';
  const univ = document.getElementById('f-universitas')?.value.trim() || '';
  const jurusan = document.getElementById('f-jurusan')?.value.trim() || '';
  const semester = document.getElementById('f-semester')?.value || '';
  const checks = [
    !!nama, !!univ, !!jurusan, !!semester,
    !!photoDataURL,
    skillTags.length > 0,
    minatTags.length > 0,
    !!cvData,
...
window.toggleEditMode = function() {
  if (isEditMode) {
    doSave(false); // simpan tanpa validasi ketat
  } else {
    isEditMode = true;
    applyEditMode();
  }
...
async function loadProfile() {
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (namaEl) namaEl.value = profile?.name || user.displayName || '';
...
document.addEventListener('DOMContentLoaded', () => {
  // Pastikan user sudah login (auth state akan menangani redirect)
  loadProfile();
  initPhotoSection();

  // Event listeners untuk tag input
